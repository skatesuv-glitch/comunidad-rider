...
function home(){shell(`<div class="communityHeader"><div class="brandLogo"><strong>eSKATE SUV</strong><span>COMUNIDAD</span></div><small>RIDERS · RUTAS · EXPERIENCIAS</small></div><h1>Comunidad Rider</h1><p class="sub">Conecta, rueda y comparte.</p><div class="backendStatus" id="backendStatus"><i></i><span>Comprobando backend…</span></div><button class="mini accountBtn" id="account" aria-label="Mi cuenta">👤</button>
${[['🎙️','Rider Voz','Conversación entre riders durante la ruta.'],['📍','Riders en mi zona','Descubre riders activos y contactos cercanos.'],['🗺️','Rutas compartidas','Descubre y comparte rutas de la comunidad.'],['🏆','Retos','Retos de distancia, desnivel y exploración.'],['👤','Mi perfil Rider','Tu identidad dentro de la comunidad.']].map((x,i)=>`<button class="card menuCard photoCard" data-menu="${i}"><span class="row"><span class="icon">${x[0]}</span><span><strong>${x[1]}</strong><small>${x[2]}</small></span></span></button>`).join('')}`);
