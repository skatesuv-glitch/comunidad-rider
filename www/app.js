...
function challengeDetail(id){const pool=window.communityChallenges||challengeData;const x=pool.find(v=>String(v.id)===String(id))||challengeData.find(v=>String(v.id)===String(id));if(!x){challenges();return}const target=Math.max(1,Number(x.target)||1),progress=Math.max(0,Number(x.progress)||0);const pct=Math.min(100,Math.round(progress/target*100));shell(topbar('RETO',true)+'<div class="challengeBadge"><span>'+pct+'%</span><small>PROGRESO</small></div><h1 class="center">'+esc(x.name)+'</h1><p class="sub center">'+esc(x.desc)+'</p><div class="bigProgress">'+pct+'%</div><span class="progress"><i style="width:'+pct+'%"></i></span><p class="center">'+progress+' / '+target+' '+esc(x.unit)+'</p><button class="btn" id="join">Unirme al reto</button>');document.querySelector('#back').onclick=challenges;const join=document.querySelector('#join');if(joinedChallenge(x.id)){join.textContent='Reto activo';join.disabled=true}join.onclick=async e=>{e.currentTarget.disabled=true;e.currentTarget.textContent='Uniéndome…';const remote=await joinChallenge(x.id);e.currentTarget.textContent=remote||!supabaseClient?'Reto activo':'Reto activo · guardado local'}}
function privacy(returnTo=map){shell(topbar('PRIVACIDAD',true)+'<div class="screenHero privacyHero"><div><small>CONTROL RIDER</small><h1>Ubicación</h1><p>Tú decides cuándo apareces en la comunidad.</p></div></div><div class="card"><h3>Compartir mi ubicación</h3><p id="shareStatus">'+(state.locationSharing?'Activada':'Desactivada')+'</p><button class="btn secondary" id="toggleShare">'+(state.locationSharing?'Desactivar':'Activar')+'</button></div><p class="sub">Al desactivarla, Comunidad deja de solicitar tu ubicación. Puedes volver a activarla cuando quieras.</p>');document.querySelector('#back').onclick=returnTo;document.querySelector('#toggleShare').onclick=()=>{if(state.locationSharing){state.locationSharing=false;saveState();clearRiderLocation().finally(()=>privacy(returnTo));return}const btn=document.querySelector('#toggleShare');if(btn){btn.disabled=true;btn.textContent='Activando…'}requestLocation(pos=>{if(!pos){state.locationSharing=false;saveState();if(btn){btn.disabled=false;btn.textContent='Activar'}const status=document.querySelector('#shareStatus');if(status)status.textContent='No se pudo obtener la ubicación';return}state.locationConsent=true;state.locationSharing=true;saveState();publishRiderLocation(pos).finally(()=>privacy(returnTo))})}}
