...
async function riderVoice(){
  ...
  const storedVoiceGroup=dbLoad().voiceGroup||[];const selected=new Map(storedVoiceGroup.map(id=>{const r=selectable.find(x=>String(x.id)===String(id));return r?[String(r.id),r]:null}).filter(Boolean));
