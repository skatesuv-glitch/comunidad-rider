...
async function ensureVoiceInbox(){
  ...
  channel.on('broadcast',{event:'voice-invite'},({payload})=>{
    ...
    if(window.riderVoiceRtc||sessionStorage.getItem('rider_voice_session')){channel.send({type:'broadcast',event:'voice-invite-response',payload:{from:me,to:String(payload.from),sessionId:payload.sessionId,accepted:false,reason:'busy'}});return}
...
function activityScreen(){const db=dbLoad();const fav=new Set((db.favoriteRoutes||[]).map(String)).size,joined=new Set((db.joinedChallenges||[]).map(String)).size;const group=new Set((db.voiceGroup||[]).map(String)).size;shell(`${topbar('ACTIVIDAD',true)}<div class="screenHero activityHero"><div><small>TU COMUNIDAD</small><h1>Actividad Rider</h1><p>Un resumen de lo que tienes en marcha.</p></div></div><div class="activityList"><button class="card activityCard" id="activityVoice"><span><small>RIDER VOZ</small><strong>${group?group+' Rider'+(group===1?'':'s')+' en tu grupo':'Sin grupo activo'}</strong></span><b>›</b></button><button class="card activityCard" id="activityRoutes"><span><small>RUTAS GUARDADAS</small><strong>${fav} favorita${fav===1?'':'s'}</strong></span><b>›</b></button><button class="card activityCard" id="activityChallenges"><span><small>RETOS ACTIVOS</small><strong>${joined} reto${joined===1?'':'s'}</strong></span><b>›</b></button></div>`);document.querySelector('#back').onclick=home;document.querySelector('#activityVoice').onclick=riderVoice;document.querySelector('#activityRoutes').onclick=sharedRoutes;document.querySelector('#activityChallenges').onclick=challenges}
...
sw.onclick=()=>{sw.classList.add('on');box.classList.remove('hidden')};document.querySelector('#accept').onclick=()=>{state.locationConsent=true;state.locationSharing=true;saveState();const btn=document.querySelector('#accept');if(btn){btn.disabled=true;btn.textContent='ACTIVANDO UBICACIÓN…'}requestLocation(pos=>{if(!pos){state.locationSharing=false;saveState();if(btn){btn.disabled=false;btn.textContent='ACEPTO Y ACTIVAR'}const note=document.querySelector('#consent .sub');if(note)note.textContent='No se pudo obtener la ubicación. Revisa el permiso de ubicación del dispositivo e inténtalo de nuevo.';return}map()})};document.querySelector('#back').onclick=home}
async function map(){
...
async function voiceInvite(r){const current=(window.communityRiders||riders).find(x=>String(x.id)===String(r?.id));if(!r||!current||current.state!=='green'){if(r?.id)profile(r.id);else map();return}r=current;shell(topbar('RIDER VOZ',true)+'<div class="voiceInvitePanel"><div class="profileAvatar voiceInviteAvatar"><span class="voiceInviteGlyph"></span></div><small>INVITACIÓN DE VOZ</small><h1>Invitar a '+esc(r.name)+'</h1><p>La conversación comienza cuando el Rider acepta.</p><div class="inviteRider"><span class="profileMark">'+esc((r.name||'R').slice(0,1).toUpperCase())+'</span><span><strong>'+esc(r.name)+'</strong><small class="online">● En línea</small></span></div><button class="btn" id="invite">Enviar invitación</button><button class="btn secondary" id="cancel">Cancelar</button><p class="sub center" id="inviteStatus"></p></div>');const go=()=>chat(r);document.querySelector('#back').onclick=go;document.querySelector('#cancel').onclick=go;document.querySelector('#invite').onclick=async e=>{if(e.currentTarget.disabled)return;const me=await currentUserId();if(!supabaseClient||!me){document.querySelector('#inviteStatus').textContent='No se pudo identificar tu sesión.';return}const sessionId=[String(me),String(r.id)].sort().join('-')+'-'+Date.now();const inbox=supabaseClient.channel('rider-voice-inbox-'+r.id,{config:{broadcast:{self:false}}});const replies=supabaseClient.channel('rider-voice-inbox-'+me,{config:{broadcast:{self:false}}});let finished=false;const finish=()=>{if(finished)return;finished=true;inbox.unsubscribe();replies.unsubscribe()};replies.on('broadcast',{event:'voice-invite-response'},({payload})=>{if(!payload||String(payload.to)!==String(me)||payload.sessionId!==sessionId)return;if(payload.accepted){sessionStorage.setItem('rider_voice_session',sessionId);sessionStorage.setItem('rider_voice_peer',String(r.id));document.querySelector('#inviteStatus').textContent=r.name+' ha aceptado · entrando en Rider Voz…';finish();setTimeout(()=>riderVoice(),250)}else{document.querySelector('#inviteStatus').textContent=payload.reason==='busy'?r.name+' está en otra conversación Rider Voz.':r.name+' ha rechazado la invitación.';e.currentTarget.disabled=false;e.currentTarget.textContent='Enviar invitación';finish()}}).subscribe(async replyStatus=>{if(replyStatus!=='SUBSCRIBED')return;inbox.subscribe(async status=>{if(status!=='SUBSCRIBED')return;try{await inbox.send({type:'broadcast',event:'voice-invite',payload:{from:me,to:String(r.id),sessionId,createdAt:Date.now()}});e.currentTarget.textContent='Invitación enviada';document.querySelector('#cancel').textContent='Volver al chat';document.querySelector('#inviteStatus').textContent='Esperando respuesta del Rider.';setTimeout(()=>{if(!finished){document.querySelector('#inviteStatus').textContent='Sin respuesta · puedes volver a intentarlo.';e.currentTarget.disabled=false;e.currentTarget.textContent='Enviar invitación';finish()}},30000)}catch{e.currentTarget.disabled=false;e.currentTarget.textContent='Enviar invitación';document.querySelector('#inviteStatus').textContent='No se pudo enviar la invitación.';finish()}})})}}
...
function privacy(returnTo=map){shell(topbar('PRIVACIDAD',true)+'<div class="screenHero privacyHero"><div><small>CONTROL RIDER</small><h1>Ubicación</h1><p>Tú decides cuándo apareces en la comunidad.</p></div></div><div class="card"><h3>Compartir mi ubicación</h3><p id="shareStatus">'+(state.locationSharing?'Activada':'Desactivada')+'</p><button class="btn secondary" id="toggleShare">'+(state.locationSharing?'Desactivar':'Activar')+'</button></div><p class="sub">Al desactivarla, Comunidad deja de solicitar tu ubicación. Puedes volver a activarla cuando quieras.</p>');document.querySelector('#back').onclick=returnTo;document.querySelector('#toggleShare').onclick=()=>{if(state.locationSharing){state.locationSharing=false;saveState();privacy(returnTo);return}const btn=document.querySelector('#toggleShare');if(btn){btn.disabled=true;btn.textContent='Activando…'}requestLocation(pos=>{if(!pos){state.locationSharing=false;saveState();if(btn){btn.disabled=false;btn.textContent='Activar'}const status=document.querySelector('#shareStatus');if(status)status.textContent='No se pudo obtener la ubicación';return}state.locationConsent=true;state.locationSharing=true;saveState();privacy(returnTo)})}}
