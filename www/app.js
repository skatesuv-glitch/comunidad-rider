...
async function enterAppAfterSplash(){
  ...
  await ensureProfile();await setRiderPresence(true);startRiderPresenceHeartbeat();await ensureVoiceInbox();const recovered=await recoverPendingVoiceInvite();if(!recovered)riderVoice();
}
...
async function riderVoice(){
  ...
  document.querySelector('#mute').onclick=e=>{e.currentTarget.classList.toggle('muted');e.currentTarget.querySelector('small').textContent=e.currentTarget.classList.contains('muted')?'Micrófono OFF':'Micrófono ON'};const volume=document.querySelector('#volume');let appVolume=Number(localStorage.getItem('rider_voice_volume')||1);const applyVolume=()=>{document.querySelectorAll('audio[data-voice-rider]').forEach(a=>{a.volume=appVolume;a.muted=appVolume===0;if(appVolume>0)a.play().catch(()=>{})});volume.classList.toggle('muted',appVolume===0);volume.querySelector('small').textContent=appVolume===0?'Audio OFF':'Volumen '+Math.round(appVolume*100)+'%';localStorage.setItem('rider_voice_volume',String(appVolume))};volume.onclick=()=>{appVolume=appVolume>0?0:1;applyVolume()};applyVolume();
  ...
  let voiceActive=false,voiceChecking=false,voxEnabled=false,voiceRecognition=null;
  ...
  const commandStatus=document.querySelector('#voiceCommandStatus'),commandToggle=document.querySelector('#voiceCommands');
  const runVoiceCommand=raw=>{const command=String(raw||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();if(!command.includes('rider'))return;const order=command.slice(command.indexOf('rider')+5).trim();commandStatus.textContent='Orden: '+(order||'esperando…');if(order.includes('subir volumen')){appVolume=Math.min(1,appVolume+.2);applyVolume();say('Volumen '+Math.round(appVolume*100)+' por ciento')}else if(order.includes('bajar volumen')){appVolume=Math.max(0,appVolume-.2);applyVolume();say('Volumen '+Math.round(appVolume*100)+' por ciento')}else if(order.includes('silenciar')){appVolume=0;applyVolume();say('Audio silenciado')}else if(order.includes('activar sonido')){appVolume=1;applyVolume();say('Sonido activado')}else if(order.includes('activar modo vox')){voxEnabled=true;say('Modo VOX activado')}else if(order.includes('desactivar modo vox')){voxEnabled=false;say('Modo VOX desactivado')}else if(order.includes('activar rider voz')){document.querySelector('#startVoice')?.click()}else if(order.includes('desactivar rider voz')){releaseVoiceMedia();setVoiceState('ready','Rider Voz desactivado')}else if(order.includes('quien esta conectado')){const names=[...selected.values()].map(x=>x.name);say(names.length?'Conectados: '+names.join(', '):'No hay riders conectados')}else if(order.includes('salir del grupo')){if(confirm('¿Salir del grupo Rider Voz?'))leave.click()}else if(order.includes('emergencia')){if(confirm('¿Confirmas activar la alerta de emergencia?')){say('Alerta de emergencia confirmada');setVoiceState('active','EMERGENCIA · alerta confirmada')}}else if(order.includes('repetir ultimo mensaje')){say('Todavía no hay un mensaje guardado para repetir')}else{say('Orden no reconocida')}};
  const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;
  commandToggle.onclick=()=>{if(voiceRecognition){voiceRecognition.stop();voiceRecognition=null;commandToggle.classList.remove('active');commandStatus.textContent='Desactivados';return}if(!Recognition){commandStatus.textContent='Reconocimiento no disponible en este dispositivo';return}voiceRecognition=new Recognition();voiceRecognition.lang='es-ES';voiceRecognition.continuous=true;voiceRecognition.interimResults=false;voiceRecognition.onresult=e=>{const text=e.results[e.results.length-1][0].transcript;runVoiceCommand(text)};voiceRecognition.onerror=e=>{commandStatus.textContent=e.error==='not-allowed'?'Permiso de voz denegado':'Escucha interrumpida · toca para reactivar'};voiceRecognition.onend=()=>{if(voiceRecognition)try{voiceRecognition.start()}catch{}};voiceRecognition.start();commandToggle.classList.add('active');commandStatus.textContent='Escuchando «Rider…»'};
  ...
  const stopVoiceWatch=()=>{clearInterval(voiceTimer);if(voiceRecognition){voiceRecognition.onend=null;voiceRecognition.stop();voiceRecognition=null}releaseVoiceMedia();document.removeEventListener('visibilitychange',onVoiceVisible);window.removeEventListener('online',onVoiceOnline);if(screenCleanup===stopVoiceWatch)screenCleanup=null};screenCleanup=stopVoiceWatch;
