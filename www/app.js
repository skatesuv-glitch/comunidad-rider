...
async function createVoiceInviteRecord(recipientId){
  if(!supabaseClient)return null;
  const me=await currentUserId();if(!me)return null;
  try{
    const expiresAt=new Date(Date.now()+30000).toISOString();
    const {data,error}=await supabaseClient.from('voice_invites').insert({sender_id:me,recipient_id:recipientId,status:'pending',expires_at:expiresAt}).select('id').single();
