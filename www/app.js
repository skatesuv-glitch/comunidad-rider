...
function home(){shell(`<div class="communityHeader"><div class="brandLogo"><strong>eSKATE SUV</strong><span>COMUNIDAD</span></div><button class="mini accountBtn" id="account" aria-label="Mi cuenta">👤</button></div><p class="communityTag">RIDERS · RUTAS · EXPERIENCIAS</p><div class="communityGrid">${[['voice','RIDER VOZ','Habla. Comparte. Conecta.'],['nearby','RIDERS EN MI ZONA','Encuentra riders cerca de ti.'],['routes','RUTAS COMPARTIDAS','Descubre. Guarda. Disfruta.'],['challenges','RETOS','Supera tus límites.']].map((x,i)=>`<button class="card menuCard photoCard heroCard hero-${x[0]}" data-menu="${i}"><span class="heroCopy"><strong>${x[1]}</strong><small>${x[2]}</small></span><span class="heroArrow">›</span></button>`).join('')}</div><button class="profileStrip" data-menu="4"><span class="profileMark">R</span><span><strong>MI PERFIL RIDER</strong><small>Cuenta, privacidad y preferencias</small></span><b>›</b></button>`);
...
function riderVoice(){shell('<div class="sectionEyebrow">eSKATE SUV · COMUNIDAD</div><div class="row spread"><div><small class="voiceLabel">RIDER VOZ</small><h1 class="voiceTitle">Conexión de voz</h1></div><button class="mini" id="back">‹</button></div><div class="voiceCockpit"><div class="voiceRing"><div class="voiceWave"><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div><strong>LISTO</strong><small>Rider Voz</small></div></div><div class="voiceControls"><button class="voiceControl" id="mute"><span>MIC</span><small>Micrófono</small></button><button class="voiceControl primaryVoice" id="find"><span>+</span><small>Rider</small></button><button class="voiceControl" id="volume"><span>VOL</span><small>Volumen</small></button></div><div class="voiceStatus"><i></i><span>Sin conexión activa</span></div><p class="sub center">Selecciona un rider para enviar una invitación. La conversación comienza cuando la acepta.</p>');document.querySelector('#back').onclick=home;document.querySelector('#find').onclick=consent}
boot();