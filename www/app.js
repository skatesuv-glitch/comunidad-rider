...
function authScreen(){
  ...
  function loginForm(){
    fields.innerHTML=`<label class="field">Email<input id="authEmail" type="email" autocomplete="email" placeholder="tu@email.com"></label><label class="field">Contraseña<input id="authPass" type="password" autocomplete="current-password" minlength="6" placeholder="Tu contraseña"></label><button class="btn" id="login">Entrar</button>`;
    document.querySelector('#login').onclick=async()=>{const email=document.querySelector('#authEmail').value.trim(),password=document.querySelector('#authPass').value;if(!email||!password){msg.textContent='Completa email y contraseña.';return}msg.textContent='Entrando…';try{const {error}=await supabaseClient.auth.signInWithPassword({email,password});if(error){msg.textContent=error.message;return}await ensureProfile();home()}catch(e){msg.textContent='No se pudo conectar. Revisa la conexión e inténtalo de nuevo.'}};
  }
  function signupForm(){
    fields.innerHTML=`<label class="field">Seudónimo Rider<input id="authAlias" autocomplete="nickname" maxlength="24" placeholder="Tu nombre en la comunidad"></label><label class="field">Email<input id="authEmail" type="email" autocomplete="email" placeholder="tu@email.com"></label><label class="field">Contraseña<input id="authPass" type="password" autocomplete="new-password" minlength="6" placeholder="Mínimo 6 caracteres"></label><label class="field">Verificar contraseña<input id="authPass2" type="password" autocomplete="new-password" minlength="6" placeholder="Repite la contraseña"></label><button class="btn" id="signup">Crear cuenta</button>`;
    document.querySelector('#signup').onclick=async()=>{const nick=document.querySelector('#authAlias').value.trim(),email=document.querySelector('#authEmail').value.trim(),password=document.querySelector('#authPass').value,password2=document.querySelector('#authPass2').value;if(nick.length<3){msg.textContent='El seudónimo debe tener al menos 3 caracteres.';return}if(!email){msg.textContent='Introduce tu email.';return}if(password.length<6){msg.textContent='La contraseña debe tener al menos 6 caracteres.';return}if(password!==password2){msg.textContent='Las contraseñas no coinciden.';return}msg.textContent='Creando cuenta…';try{const {data,error}=await supabaseClient.auth.signUp({email,password,options:{data:{alias:nick}}});if(error){msg.textContent=error.message;return}msg.textContent=data&&data.session?'✓ Cuenta creada':'✓ Cuenta creada. Revisa tu email para confirmarla.';if(data&&data.session){await ensureProfile();home()}}catch(e){msg.textContent='No se pudo conectar. Revisa la conexión e inténtalo de nuevo.'}};
  }
