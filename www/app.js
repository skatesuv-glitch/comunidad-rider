...
async function riderVoice(){
  ...
  const stopVoiceWatch=()=>{clearInterval(voiceTimer);if(voiceRecognition){voiceRecognition.onend=null;voiceRecognition.stop();voiceRecognition=null}if(nativeVoice&&nativeCommandsOn)nativeVoice.stopCommandListening().catch(()=>{});if(nativeCommandHandle?.remove)nativeCommandHandle.remove();releaseVoiceMedia();document.removeEventListener('visibilitychange',onVoiceVisible);window.removeEventListener('online',onVoiceOnline);if(screenCleanup===stopVoiceWatch)screenCleanup=null};screenCleanup=stopVoiceWatch;
